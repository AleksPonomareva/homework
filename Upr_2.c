...
// Функция сортировки расческой с использованием указателей
int comb_sort(char (*array)[10], const int size) {
int count = 0;
int gap = size;
float shrink = 1.3;
int swapped = 1;
...
swapped = 0;
for (char (*i)[10] = array; i + gap < array + size; i++) {
if (strcmp(*i, *(i + gap)) > 0) {
swap(array, i - array, (i + gap) - array);
swapped = 1;
}
count++;
}
...
if (choice == 1) {
    ...
    clock_t start = clock();
    int count = comb_sort(words, SIZE);
    clock_t end = clock();
    printf("Отсортированный массив:\n");
    print_array(words);
    printf("Число итераций: %d\n", count);
    ...
    int count3 = comb_sort(words, SIZE);
    clock_t end3 = clock();
